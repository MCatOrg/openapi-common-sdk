...
from example.openapi.common_client import CommonClient
...
class Credential(object):
    def __init__(self, secret_id, secret_key, token=None):
        ...
        if secret_id is None or secret_id.strip() == "":
            raise MCatSDKException("InvalidCredential", "secret id should not be none or empty")
        if secret_id.strip() != secret_id:
            raise MCatSDKException("InvalidCredential", "secret id should not contain spaces")
        self.secret_id = secret_id

        if secret_key is None or secret_key.strip() == "":
            raise MCatSDKException("InvalidCredential", "secret key should not be none or empty")
        if secret_key.strip() != secret_key:
            raise MCatSDKException("InvalidCredential", "secret key should not contain spaces")
...
class CVMRoleCredential(object):
    ...
    def get_role_name(self):
        ...
        try:
            ...
            raise MCatSDKException("ClientError.MetadataError", str(e))
        finally:
            return self.role
...
class DefaultCredentialProvider(object):
    ...
    def get_credentials(self):
        ...
        raise MCatSDKException("ClientSideError", "no valid credentail.")
