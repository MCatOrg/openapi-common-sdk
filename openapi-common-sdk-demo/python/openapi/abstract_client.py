...
class AbstractClient(object):
    ...
    def _format_params(self, prefix, params):
        ...
        raise MCatSDKException("ClientParamsError", "some params type error")

    def _build_req_inter(self, params, req_inter, options=None):
        options = options or {}
        if self.profile.signMethod in ("hmac-sha256", "hmac-sha256","hmac-sha1"):
    ...
    def _build_req_with_signature(self, params, req, options=None):
        ...
        if req.method == "GET" and content_type == _multipart_content:
            raise MCatSDKException("ClientError",
    ...
    def call_octet_stream(self, action, headers, body):
        ...
        if self.profile.httpProfile.reqMethod != "POST":
            raise MCatSDKException("ClientError", "Invalid request method.")
        ...
        if "Error" in json_rsp["Response"]:
            ...
            raise MCatSDKException(code, message, reqid)
        return json_rsp
